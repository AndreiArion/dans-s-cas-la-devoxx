...
class e1_classes extends HandsOnSuite {
  ...
  */
  exercice("Le paramètre val définit un getter") {
    val aClass = new ClassWithValParameter("qwerty")
    aClass.name should be("qwerty")
  }
  ...
  */
  exercice("Le paramètre var définit un getter et un setter") {
    val aClass = new ClassWithVarParameter("asdf")
    aClass.description should be("asdf")

    aClass.description = "wsdf"
    aClass.description should be("wsdf")
  }
